...
async function sbUploadFile(path, file){
  ...
  if(!r.ok) throw new Error(await r.text());
  return `${SB_URL}/storage/v1/object/public/${EPILASYON_BUCKET}/${path}`;
}
...
export default function App() {
  ...
  function kullaniciEtiket(){
    return aktifKullanici?.login_name?`${aktifKullanici.login_name} (${ROLLER[aktifRol]})`:ROLLER[aktifRol];
  }
  ...
  // ── Yeni sürüm kontrolü: açık kalan sekmeler eski kodda kalmasın ──
  // Sunucudaki index.html'in işaret ettiği JS dosyası, bu sayfanın yüklediğinden farklıysa üstte "Yenile" bandı çıkar.
  const [yeniSurum,setYeniSurum]=useState(false);
  useEffect(()=>{
    const benim=[...document.querySelectorAll("script[src]")].map(x=>x.getAttribute("src")).find(x=>/assets\/index-[^/]+\.js/.test(x||""));
    if(!benim)return; // geliştirme ortamı
    let iptal=false;
    async function kontrol(){
      try{
        const html=await fetch("/?v="+Date.now(),{cache:"no-store"}).then(r=>r.text());
        const sunucu=(html.match(/assets\/index-[^"']+\.js/)||[])[0];
        if(!iptal&&sunucu&&!benim.includes(sunucu))setYeniSurum(true);
      }catch{}
    }
    const t=setInterval(kontrol,5*60*1000);
    const gor=()=>{if(document.visibilityState==="visible")kontrol();};
    document.addEventListener("visibilitychange",gor);
    kontrol();
    return()=>{iptal=true;clearInterval(t);document.removeEventListener("visibilitychange",gor);};
  },[]);
  ...
  async function randevuKaydet(data){
    ...
    try{
      if(data.id){
        ...
        const aynıGunErteleme=eskiR&&data.tarih&&data.tarih!==eskiR.tarih&&eskiR.tarih===today();
        const yeniLog=[...(eskiR?.log||[]),{saat:now,kullanici:kullaniciEtiket(),islem:aynıGunErteleme?`AYNI_GUN_ERTELEME: ${eskiR.tarih} → ${data.tarih}`:"Randevu güncellendi"}];
        try{
  ...
  async function bolgeGuncelle(id,yeniBolgeler,yeniSure){
    try{
      ...
      const now=nowTime();
      const yeniLog=[...(r?.log||[]),{saat:now,kullanici:kullaniciEtiket(),islem:`Bölge güncellendi: ${yeniBolgeler.join(", ")} (${yeniSure}dk)`}];
  ...
  async function randevuTasi(randevu, yeniSaat){
    try{
      ...
      const now=nowTime();
      const yeniLog=[...(randevu.log||[]),{saat:now,kullanici:kullaniciEtiket(),islem:`Saat değiştirildi: ${randevu.saat} → ${yeniSaat}`}];
  ...
  async function randevuSil(id){
    ...
    try{
      if(r){
        const sl={sil_tarih:today(),sil_saat:nowTime(),kullanic:kullaniciEtiket(),hasta:r.hasta,oda:r.oda,randevu_tarih:r.tarih,randevu_saat:r.saat,bolgeler:r.bolgeler||[],sure:r.sure,durum:r.durum,odeme:r.odeme};
  ...
  async function beklemeNotGuncelle(id,yeniNot){
    try{
      await sbUpdate("bekleme",id,{notlar:yeniNot});
      setBekleme(prev=>prev.map(b=>b.id===id?{...b,notlar:yeniNot}:b));
    ...
    <div style={{minHeight:"100vh",background:"#f5f4f1",fontFamily:"'DM Sans',system-ui,sans-serif",overflowX:"hidden",maxWidth:"100vw"}}>
      ...
      <nav style={{background:"#fff",borderBottom:"1px solid #e8e6e0",padding:"0 1.5rem"}}>
        <div style={{maxWidth:1200,margin:"0 auto",display:"flex"}}>
          {[["takvim","📅 Takvim",0],...(aktifRol!=="sekreter"?[["bildirimler","🔔 Bildirimler",bildirimSayisi]]:[]),["bekleyen","🧍 Bekleyen Hasta",0],["hastalar","👤 Hastalar",0],["bekleme","⏳ Bekleme",beklemeSayisi],
      ...
      <div style={{maxWidth:1200,margin:"0 auto",padding:"1.25rem 1.5rem"}}>
        {aktifSekme==="takvim"&&<TakvimSekme hastalar={hastalar} seciliTarih={seciliTarih} setSeciliTarih={setSeciliTarih} alexR={alexR} sopR={sopR} gunB={gunB} bloklar={bloklar} blokEkle={blokEkle} blokSil={blokSil} randevular={randevular} aktifRol={aktifRol} onYeniRandevu={(oda,saat)=>setModal({tip:"yeni",data:{oda,saat,tarih:seciliTarih}})} onRandevuTikla={r=>setModal({tip:"detay",data:r})} onRandevuDuzenle={r=>setModal({tip:"duzenle",data:r})} onRandevuTasi={randevuTasi} showToast={showToast} epilasyonDurum={epilasyonDurum}/>}
        ...
        {aktifSekme==="bekleyen"&&<BekleyenHastaSekme randevular={randevular} aktifRol={aktifRol} aktifKullanici={aktifKullanici} showToast={showToast}/>}
      ...
      {yeniSurum&&<div onClick={()=>window.location.reload()} style={{position:"fixed",top:0,left:0,right:0,zIndex:10000,background:"#4338ca",color:"#fff",padding:"10px 16px",fontSize:14,fontWeight:600,textAlign:"center",cursor:"pointer",boxShadow:"0 2px 10px rgba(0,0,0,0.2)"}}>🔄 Sistemin yeni sürümü yüklendi — güncellemek için dokunun</div>}
...
// ── TAKVİM ───────────────────────────────────────────────────────────────────
function TakvimSekme({hastalar=[],seciliTarih,setSeciliTarih,alexR,sopR,gunB,bloklar,blokEkle,blokSil,randevular,aktifRol,onYeniRandevu,onRandevuTikla,showToast,onRandevuDuzenle,onRandevuTasi,epilasyonDurum}){
  ...
  function bosluklariBul(randevular,bloklar,bitisSaat){
    ...
    if(imlec<bitisSaat) bosluklar.push({b:imlec,e:bitisSaat});
  ...
  function digerOdaDurumu(saat,digerOda){
    ...
    const digerBloklar=gunB.filter(b=>b.oda===digerOda&&b.baslik!=="DR_YOK");
    const dakika=timeToMin(saat);
    const doluRandevu=digerRandevular.find(r=>{const b=timeToMin(r.saat),e=b+r.sure;return dakika>=b&&dakika<e;});
    if(doluRandevu)return{durum:"dolu",metin:doluRandevu.hasta};
    const doluBlok=digerBloklar.find(b=>{const bb=timeToMin(b.saat),be=bb+b.sure;return dakika>=bb&&dakika<be;});
    if(doluBlok)return{durum:"blok",metin:doluBlok.baslik};
    return{durum:"bos"};
  }
        ...
        <div style={{display:"flex",gap:10}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            {satirlar.map((s,i)=>{
              if(s.tip==="bosluk"){
                ...
                return(
                  <div key={"bo"+i} onClick={()=>onYeniRandevu(odaId,minToTime(s.b))} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 10px",marginBottom:5,borderRadius:6,background:"#fff8dc",cursor:"pointer"}}>
                    <div style={{fontSize:11,fontWeight:800,color:"#8a6d1f",width:42,flexShrink:0}}>{minToTime(s.b)}</div>
                    ...
                    {(()=>{
                      ...
                      return(
                        <span style={{background:"#dc2626",color:"#fff",fontSize:9,fontWeight:700,padding:"3px 8px",borderRadius:999,flexShrink:0,whiteSpace:"nowrap"}} title={`${kisaAd} — ${minToTime(s.b)}: ${dd.metin}`}>
                          {kisaAd} D
                        </span>
                      );
                    })()}
              ...
              if(s.tip==="blok"){
                return(
                  <div key={"bl"+i} onClick={()=>{if(window.confirm(`"${s.veri.baslik}" bloğunu sil?`))blokSil(s.veri.id);}} style={{display:"flex",alignItems:"center",gap:8,padding:"8px 10px",marginBottom:5,borderRadius:8,background:"repeating-linear-gradient(45deg,#888,#888 4px,#aaa 4px,#aaa 8px)",border:"1px solid #666",cursor:"pointer"}}>
                    <div style={{fontSize:11,fontWeight:800,color:"#fff",width:42,flexShrink:0}}>{minToTime(s.b)}</div>
                    <div style={{fontSize:12,fontWeight:600,color:"#fff"}}>🔒 {s.veri.baslik} ({s.e-s.b}dk)</div>
                  </div>
                );
              }
              const u=s.veri;
              ...
              const gunSonuGecti=r=>{
                if(r.tarih<today())return true; // gün tamamen geçmiş
              ...
              return(
                <div key={u.id} onClick={()=>{if(u.list.length===1)onRandevuTikla(u.list[0]);else setKumePopup({liste:u.list});}}
                  ...
                  {(()=>{
                    ...
                    if(!doluMu)return null;
                    return(
                      <span style={{background:"#dc2626",color:"#fff",fontSize:9,fontWeight:700,padding:"3px 8px",borderRadius:999,flexShrink:0,whiteSpace:"nowrap"}} title={`${kisaAd} — ${u.saat}: ${dd.metin}`}>
                        {kisaAd} D
                      </span>
                    );
                  })()}
...
// ── EPİLASYON KARTI ──────────────────────────────────────────────────────────
function BolgeSatiri({satir,onDegis,onSil,kilitli}){
  return(
    <div style={{border:"1px solid "+(kilitli?"#c7d2fe":"#e8e6e0"),background:kilitli?"#eef0ff":"#fff",borderRadius:10,padding:"10px 12px",marginBottom:10}}>
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
        {kilitli&&<span style={{color:"#6366f1",fontSize:13}}>✓</span>}
        <span style={{fontSize:13,fontWeight:600,color:kilitli?"#4338ca":"#333"}}>{satir.bolge}</span>
        {kilitli&&<span style={{fontSize:11,color:"#888",marginLeft:"auto"}}>randevudan geldi</span>}
        {!kilitli&&<button onClick={onSil} style={{marginLeft:"auto",background:"none",border:"none",color:"#dc2626",fontSize:12,cursor:"pointer",fontFamily:"inherit"}}>Kaldır</button>}
      </div>
      <select value={satir.cihaz} onChange={e=>onDegis({...satir,cihaz:e.target.value})} style={{...inputStyle,marginBottom:6,fontSize:13,padding:"7px 8px"}}>
        {EPILASYON_CIHAZLAR.map(c=><option key={c} value={c}>{c}</option>)}
      </select>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:6}}>
        <input inputMode="decimal" placeholder="Enerji (j/cm²)" value={satir.enerji} onChange={e=>onDegis({...satir,enerji:e.target.value})} style={{...inputStyle,fontSize:13,padding:"7px 8px"}}/>
        <input inputMode="numeric" placeholder="ms" value={satir.ms} onChange={e=>onDegis({...satir,ms:e.target.value})} style={{...inputStyle,fontSize:13,padding:"7px 8px"}}/>
        <input inputMode="numeric" placeholder="Atım" value={satir.atim} onChange={e=>onDegis({...satir,atim:e.target.value})} style={{...inputStyle,fontSize:13,padding:"7px 8px"}}/>
      </div>
      <div style={{display:"flex",gap:5,marginBottom:6,flexWrap:"wrap"}}>
        {DOKULME_SECENEKLERI.map(d=><button key={d} onClick={()=>onDegis({...satir,dokulme:d})} style={{...chipStyle(satir.dokulme===d),fontSize:11,padding:"5px 10px"}}>{d}</button>)}
      </div>
      <input placeholder="Not (kızarıklık, dökülme durumu...)" value={satir.not} onChange={e=>onDegis({...satir,not:e.target.value})} style={{...inputStyle,fontSize:12,padding:"7px 8px"}}/>
    </div>
  );
}
...
function YeniSeansForm({randevu,aktifKullanici,aktifRol,onKaydet,onKasayaGonder,onIptal,showToast}){
  ...
  const [satirlar,setSatirlar]=useState(()=>{
    if(randevu?.bolgeler?.length){
      return randevu.bolgeler.map(b=>({bolge:b,cihaz:varsayilanCihaz,enerji:"",ms:"",atim:"",dokulme:"",not:"",kilitli:true}));
    }
    return [];
  });
  const [ekBolgePicker,setEkBolgePicker]=useState(false);
  ...
  useEffect(()=>{
    async function yukle(){
      try{
        const p=await sbGet("personel_listesi","select=login_name");
        const isimler=p.map(x=>x.login_name).filter(Boolean);
        setPersonelListesi(isimler.length?isimler:EPILASYON_UYGULAYICI_YEDEK);
      }catch(e){setPersonelListesi(EPILASYON_UYGULAYICI_YEDEK);}
    }
    yukle();
  },[]);

  function satirGuncelle(idx,yeni){setSatirlar(s=>s.map((x,i)=>i===idx?yeni:x));}
  function satirSil(idx){setSatirlar(s=>s.filter((_,i)=>i!==idx));}
  function bolgeEkle(b){setSatirlar(s=>[...s,{bolge:b,cihaz:varsayilanCihaz,enerji:"",ms:"",atim:"",dokulme:"",not:"",kilitli:false}]);setEkBolgePicker(false);}

  return(
    <div style={{border:"1px solid #e8e6e0",borderRadius:10,padding:12,marginBottom:14,background:"#fafaf8"}}>
      <Label>Tarih</Label>
      <input type="date" value={tarih} onChange={e=>setTarih(e.target.value)} style={{...inputStyle,marginBottom:10}}/>
      <Label>Uygulayıcı *</Label>
      <select value={uygulayici} onChange={e=>setUygulayici(e.target.value)} style={{...inputStyle,marginBottom:10,border:!uygulayici?"1.5px solid #f59e0b":inputStyle.border}}>
        <option value="">Seçiniz...</option>
        {aktifRol==="personel"&&aktifKullanici?.login_name&&!personelListesi.includes(aktifKullanici.login_name)&&<option value={aktifKullanici.login_name}>{aktifKullanici.login_name}</option>}
        {personelListesi.map(p=><option key={p} value={p}>{p}</option>)}
      </select>
      {satirlar.length===0&&<div style={{color:"#aaa",fontSize:13,marginBottom:8}}>Bölge seçilmedi.</div>}
      {satirlar.map((s,i)=>(
        <BolgeSatiri key={i} satir={s} kilitli={s.kilitli} onDegis={y=>satirGuncelle(i,y)} onSil={()=>satirSil(i)}/>
      ))}
      {!ekBolgePicker?(
        <button onClick={()=>setEkBolgePicker(true)} style={{...btnSecondary,width:"100%",borderStyle:"dashed",marginBottom:12}}>+ Ek bölge işaretle</button>
      ):(
        <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:12}}>
          {bolgeListesi.filter(b=>!satirlar.find(s=>s.bolge===b)).map(b=>(
            <button key={b} onClick={()=>bolgeEkle(b)} style={chipStyle(false)}>{b}</button>
          ))}
        </div>
      )}
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
...
function EpilasyonKart({hasta,randevu,aktifKullanici,aktifRol,onKapat,onKasayaGonder,showToast}){
  ...
  useEffect(()=>{
    ...
    async function yukle(){
      setYukleniyor(true);
      try{
        const g=await sbGet("epilasyon_gecmis",`hasta_id=eq.${hasta.id}`);
        if(iptal)return;
        setGecmis(g[0]||null);
        setGecmisNot(g[0]?.gecmis_notu||"");
        setGecmisFotolar(g[0]?.foto_urls||[]);
        const z=await sbGet("epilasyon_ziyaretleri",`hasta_id=eq.${hasta.id}&select=*,epilasyon_bolge_uygulamalari(*)&order=tarih.desc`);
        if(iptal)return;
        setZiyaretler(z);
      }catch(e){showToast("Epilasyon verisi yüklenemedi: "+e.message,"error");}
  ...
  async function fotoYukle(dosyalar){
    ...
    try{
      ...
      const tumFotolar=[...gecmisFotolar,...yeniUrller];
      setGecmisFotolar(tumFotolar);
      if(gecmis){await sbUpdate("epilasyon_gecmis",gecmis.id,{foto_urls:tumFotolar});}
      else{const [ins]=await sbInsert("epilasyon_gecmis",{hasta_id:hasta.id,foto_urls:tumFotolar,gecmis_notu:gecmisNot});setGecmis(ins);}
      showToast("Fotoğraf eklendi.");
    }catch(e){showToast("Yükleme hatası: "+e.message,"error");}
    finally{setYukleniyorFoto(false);}
  }

  async function gecmisNotKaydet(){
    try{
      if(gecmis){await sbUpdate("epilasyon_gecmis",gecmis.id,{gecmis_notu:gecmisNot});}
      else{const [ins]=await sbInsert("epilasyon_gecmis",{hasta_id:hasta.id,foto_urls:gecmisFotolar,gecmis_notu:gecmisNot});setGecmis(ins);}
    ...
    try{
      ...
      for(const s of satirlar){
        await sbInsert("epilasyon_bolge_uygulamalari",{
          ...
          enerji_joule:s.enerji?parseFloat(s.enerji):null,
          pulse_suresi_ms:s.ms?parseFloat(s.ms):null,
          atim_sayisi:s.atim?parseInt(s.atim):null,
          tuy_dokulme_degerlendirmesi:s.dokulme||null,
          not_metni:s.not||""
        });
      }
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
        <div>
          <div style={{fontSize:17,fontWeight:600}}>{hasta.ad}</div>
          {hasta.hasta_id&&<div style={{fontSize:12,color:"#888"}}>ID: #{hasta.hasta_id}</div>}
        </div>
        <button onClick={onKapat} style={btnSecondary}>Kapat</button>
      </div>

      {yukleniyor?<div style={{textAlign:"center",color:"#aaa",padding:20}}>Yükleniyor...</div>:(
        <>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
            <div style={{background:"#f7f7f5",borderRadius:10,padding:"8px 10px"}}>
              <div style={{fontSize:11,color:"#888"}}>Toplam seans</div>
              <div style={{fontSize:17,fontWeight:600}}>{toplamSeans}</div>
            </div>
            <div style={{background:"#f7f7f5",borderRadius:10,padding:"8px 10px"}}>
              <div style={{fontSize:11,color:"#888"}}>Son ziyaret</div>
              <div style={{fontSize:17,fontWeight:600}}>{sonZiyaret||"—"}</div>
            </div>
          </div>
          ...
          <div style={{border:"1px solid #e8e6e0",borderRadius:10,padding:12,marginBottom:14}}>
            ...
            <label style={{display:"inline-block",padding:"8px 14px",border:"1px dashed #bbb",borderRadius:8,fontSize:12,color:"#666",cursor:"pointer",marginBottom:8}}>
              {yukleniyorFoto?"Yükleniyor...":"📷 Fotoğraf ekle"}
              <input type="file" accept="image/*" capture="environment" multiple style={{display:"none"}} disabled={yukleniyorFoto}
                onChange={e=>{if(e.target.files.length)fotoYukle(Array.from(e.target.files));e.target.value="";}}/>
            </label>
            {gecmisForm?(
              <div>
                <textarea rows={2} value={gecmisNot} onChange={e=>setGecmisNot(e.target.value)} placeholder="Geçmiş notu (örn. 9 seans, Soprano+Alex, iyi yanıt)" style={{...inputStyle,marginBottom:8,resize:"none"}}/>
                <div style={{display:"flex",gap:8}}>
                  <button onClick={gecmisNotKaydet} style={btnPrimary}>Kaydet</button>
                  <button onClick={()=>setGecmisForm(false)} style={btnSecondary}>İptal</button>
                </div>
              </div>
            ):(
              <div onClick={()=>setGecmisForm(true)} style={{fontSize:13,color:gecmisNot?"#333":"#aaa",cursor:"pointer",padding:"6px 0"}}>
                {gecmisNot||"+ Geçmiş notu ekle"}
              </div>
            )}
          </div>
        ...
        <div style={{flex:1}}>
          ...
          {notDuzenle&&(
            <div style={{marginTop:8,display:"flex",gap:8,alignItems:"flex-start"}}>
              <textarea value={notTaslak} onChange={e=>setNotTaslak(e.target.value)} rows={2} placeholder="Not ekle..." style={{...inputStyle,resize:"vertical",flex:1}}/>
              <div style={{display:"flex",flexDirection:"column",gap:6}}>
                <button onClick={async()=>{await onNotGuncelle(b.id,notTaslak.trim());setNotDuzenle(false);}} style={{...btnPrimary,fontSize:12,padding:"6px 12px",whiteSpace:"nowrap"}}>Kaydet</button>
                <button onClick={()=>{setNotTaslak(b.notlar||"");setNotDuzenle(false);}} style={{...btnSecondary,fontSize:12,padding:"6px 12px",whiteSpace:"nowrap"}}>İptal</button>
              </div>
            </div>
          )}
        </div>
...
function BildirimlerSekme({randevular,hastalar,aktifRol,onRandevuTikla,onEpilasyonAc}){
  ...
  const durumEksik=randevular
    .filter(r=>r.tarih<=bugun&&r.tarih>=BILDIRIM_TAKIP_BASLANGIC&&!(r.log||[]).some(l=>l.islem?.includes("Durum:"))&&bildirimGorunurMu(r,aktifRol))
...
function HastalarSekme({hastalar,hastaEkleDB,hastaGuncelle,aktifRol,showToast,randevular,silLog,onRandevuDuzenle,onRandevuSil,onEpilasyonAc}){
  ...
  const ayniGunSilinen=seciliHasta?silLog.filter(s=>{
    const ad=s.hasta?.toLowerCase().trim()===seciliHasta.ad?.toLowerCase().trim();
    const rt=s.randevuTarih||s.randevu_tarih, st=s.silTarih||s.sil_tarih;
    return ad&&rt&&st&&rt===st;
  }):[];
  const ayniGunErtelenen=hastaRandevular.flatMap(r=>(r.log||[]).filter(l=>l.islem?.startsWith("AYNI_GUN_ERTELEME")).map(l=>({...l,rid:r.id})));
  const haberVerdiListesi=[...ayniGunSilinen,...ayniGunErtelenen];
  const bugun=today();
        ...
        <div>
          ...
          <div style={{background:"#fff",border:"1px solid #e8e6e0",borderRadius:12,padding:"1.25rem",marginBottom:14}}>
            <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12,flexWrap:"wrap"}}>
              <button onClick={()=>setGelmedPopup(true)} style={{background:gelmedListesi.length>=2?"#dc2626":"#f0f0ed",color:gelmedListesi.length>=2?"#fff":"#666",border:"none",borderRadius:20,padding:"6px 14px",fontSize:12,fontWeight:700,cursor:"pointer"}}>
                🚫 Gelmedi: {gelmedListesi.length}
              </button>
              {haberVerdiListesi.length>0&&(
                <span onClick={()=>setGelmedPopup(true)} style={{fontSize:11,color:"#2563eb",background:"#eff6ff",borderRadius:20,padding:"5px 12px",fontWeight:600,cursor:"pointer"}}>
                  📞 {haberVerdiListesi.length}
                </span>
              )}
            </div>
            ...
            <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
              ...
              <button onClick={()=>onEpilasyonAc(seciliHasta)} style={{...btnSecondary,color:"#6366f1",borderColor:"#a5b4fc",marginLeft:"auto"}}>📋 Epilasyon Kartı</button>
            </div>
          </div>

          {gelmedPopup&&(
            <div onClick={()=>setGelmedPopup(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.25)",zIndex:200,display:"flex",alignItems:"flex-start",justifyContent:"center",paddingTop:"12vh"}}>
              <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:14,padding:16,width:320,maxHeight:"60vh",overflowY:"auto",boxShadow:"0 10px 40px rgba(0,0,0,0.3)"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:10}}>
                  <span style={{fontWeight:700,fontSize:14}}>{seciliHasta.ad}</span>
                  <span onClick={()=>setGelmedPopup(false)} style={{cursor:"pointer",color:"#999",fontSize:16,padding:"0 4px"}}>✕</span>
                </div>
                <div style={{fontSize:12,fontWeight:700,color:"#dc2626",marginBottom:6}}>🚫 Gelmedi ({gelmedListesi.length})</div>
                {gelmedListesi.length===0?(
                  <div style={{fontSize:12,color:"#aaa",marginBottom:12}}>Kayıt yok (16 Temmuz 2026 sonrası).</div>
                ):gelmedListesi.map(r=>(
                  <div key={r.id} style={{fontSize:12,padding:"6px 8px",background:"#fef2f2",borderRadius:6,marginBottom:4}}>{r.tarih} · {r.saat} · {r.oda==="alex"?"Alex":"Soprano"}</div>
                ))}
                <div style={{fontSize:12,fontWeight:700,color:"#2563eb",marginTop:12,marginBottom:6}}>📞 Randevu Günü Haber Verdi ({haberVerdiListesi.length})</div>
                {haberVerdiListesi.length===0?(
                  <div style={{fontSize:12,color:"#aaa"}}>Kayıt yok.</div>
                ):haberVerdiListesi.map((s,i)=>{
                  const silmeMi=s.randevu_tarih!==undefined||s.randevuTarih!==undefined;
                  if(silmeMi){
                    return(
                      <div key={"s"+(s.id??i)} style={{fontSize:12,padding:"6px 8px",background:"#eff6ff",borderRadius:6,marginBottom:4}}>
                        {s.randevuTarih||s.randevu_tarih} · {s.randevuSaat||s.randevu_saat} · {s.oda==="alex"?"Alex":"Soprano"} · aynı gün silindi
                      </div>
                    );
                  }
                  const esk=s.islem.match(/AYNI_GUN_ERTELEME: (.+) → (.+)/);
                  return(
                    <div key={"e"+s.rid+i} style={{fontSize:12,padding:"6px 8px",background:"#eff6ff",borderRadius:6,marginBottom:4}}>
                      {esk?.[1]} · aynı gün başka tarihe alındı ({esk?.[2]})
                    </div>
                  );
                })}
              </div>
...
function DashboardSekme({randevular,bloklar,bekleme,setSeciliTarih,setAktifSekme,onYeniRandevu}){
  ...
  function gunBoslukBul(tarih){
    ...
    ["alex","soprano"].forEach(oda=>{
      ...
      if(imlec<kapanis-5) sonuc[oda].push({saat:minToTime(imlec),dk:kapanis-imlec});
    });
    return sonuc;
  }
...
function AnketSonucSekme({aktifRol}){
  ...
  const [anketSekme,setAnketSekme]=useState("puanlar");
  ...
  const gosterilenFiltreli=(()=>{
    let liste;
    if(filtre==="9ustu")liste=yuksekPuan;
    else if(filtre==="8alti")liste=sirali.filter(a=>a.puan>0&&a.puan<8);
    else if(filtre==="5alti")liste=cokDusukPuan;
    else liste=sirali.filter(a=>a.puan>0);
  ...
  return(
    <div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap"}}>
        {[
          {id:"puanlar",label:"📊 Puanlamalar",sayi:anketler.length},
          {id:"gorusler",label:"💬 Hasta Görüşleri",sayi:yorumlar.length},
          {id:"istatistikler",label:"📈 İstatistikler"},
        ].map(s=>(
          <button key={s.id} onClick={()=>setAnketSekme(s.id)} style={{padding:"10px 18px",borderRadius:10,border:anketSekme===s.id?"2px solid #6366f1":"1px solid #ddd",background:anketSekme===s.id?"#eef0ff":"#fff",color:anketSekme===s.id?"#6366f1":"#555",fontWeight:anketSekme===s.id?700:400,fontSize:14,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",gap:6}}>
            {s.label}{s.sayi!==undefined&&<span style={{background:anketSekme===s.id?"#6366f1":"#e8e6e0",color:anketSekme===s.id?"#fff":"#888",fontSize:11,fontWeight:700,padding:"1px 7px",borderRadius:20}}>{s.sayi}</span>}
          </button>
        ))}
      </div>
      ...
      {anketSekme==="puanlar"&&(<>
                ...
                return(
                  <div key={a.id||i} style={{padding:"10px 16px",borderBottom:i<cokDusukPuan.length-1?"1px solid #f5f5f2":"none"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                      <div><span style={{fontWeight:600,fontSize:13}}>{a.hasta}</span><span style={{marginLeft:8,fontSize:20,fontWeight:700,color:"#dc2626"}}>{a.puan}/10</span></div>
                      <span style={{fontSize:11,color:"#888"}}>{a.tamamlama_tarih}</span>
                    </div>
                    <div style={{fontSize:12,color:"#888",marginTop:2}}>{a.oda==="alex"?"Alex Lazer":a.oda==="soprano"?"Soprano":"—"}{a.randevu_tarih&&<span> · Randevu: {a.randevu_tarih}</span>}{personelTahmini&&<span style={{color:"#7c3aed",fontWeight:600}}> · 👤 {personelTahmini} (tahmini)</span>}</div>
                    {a.cevaplar&&Object.entries(a.cevaplar).filter(([k,v])=>k.startsWith("s")&&v).map(([k,v])=>(<div key={k} style={{fontSize:11,color:"#666",marginTop:2}}><span style={{color:"#aaa"}}>{soruMetni(a.anket_tipi,k)}:</span> <strong>{v}</strong></div>))}
                  </div>
                );
              })}
            </div>
        ...
        {!sekreterModu&&<div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
          ...
          <button onClick={()=>{setFiltre("9ustu");setAnketArama("");}} style={{...chipStyle(filtre==="9ustu"&&!anketArama),fontSize:13,background:filtre==="9ustu"&&!anketArama?"#f0fdf4":undefined,color:filtre==="9ustu"&&!anketArama?"#16a34a":undefined,border:filtre==="9ustu"&&!anketArama?"1.5px solid #86efac":undefined}}>⭐ 9 üstü ({yuksekPuan.length})</button>
          <button onClick={()=>{setFiltre("8alti");setAnketArama("");}} style={{...chipStyle(filtre==="8alti"&&!anketArama),fontSize:13,background:filtre==="8alti"&&!anketArama?"#fff7ed":undefined,color:filtre==="8alti"&&!anketArama?"#c2410c":undefined,border:filtre==="8alti"&&!anketArama?"1.5px solid #fed7aa":undefined}}>📋 8 altı ({sirali.filter(a=>a.puan>0&&a.puan<8).length})</button>
          <button onClick={()=>{setFiltre("5alti");setAnketArama("");}} style={{...chipStyle(filtre==="5alti"&&!anketArama),fontSize:13,background:filtre==="5alti"&&!anketArama?"#fef2f2":undefined,color:filtre==="5alti"&&!anketArama?"#dc2626":undefined,border:filtre==="5alti"&&!anketArama?"1.5px solid #fca5a5":undefined}}>⚠️ 5 altı ({cokDusukPuan.length})</button>
        ...
        {gosterilen.length>0&&(
          <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {gosterilen.slice(0,50).map(a=>{
              const randevuSaati=randevuSaatMap.get(a.randevu_id);
              const personel=a.oda==="alex"&&randevuSaati?alexPersoneliTahmini(a.randevu_tarih,randevuSaati):a.oda==="soprano"?"Hanife":null;
              return(
              <div key={a.id} style={{background:"#fff",border:"1px solid #e8e6e0",borderRadius:12,padding:"12px 16px"}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  <div><span style={{fontWeight:600,fontSize:14}}>{a.hasta}</span><span style={{marginLeft:10,fontSize:22,fontWeight:700,color:a.puan>=9?"#16a34a":a.puan>=7?"#f59e0b":"#dc2626"}}>{a.puan}/10</span></div>
                  <div style={{display:"flex",alignItems:"center",gap:8}}>
                    {a.puan>=9&&<button onClick={()=>googleGonder(a)} style={{...btnPrimary,fontSize:11,padding:"4px 10px",background:"#16a34a"}}>⭐ Google Yorum</button>}
                    <span style={{fontSize:11,color:"#888"}}>{a.tamamlama_tarih}</span>
                  </div>
                </div>
                <div style={{fontSize:12,color:"#888"}}>{a.anket_tipi==="lazer"?(a.oda==="alex"?"Alex Lazer":a.oda==="soprano"?"Soprano Lazer":"Lazer Epilasyon"):"Cilt/Karbon/Tüy"}{a.randevu_tarih&&<span style={{marginLeft:6,color:"#6366f1"}}>| Randevu: {a.randevu_tarih}</span>}{personel&&<span style={{marginLeft:6,color:"#7c3aed",fontWeight:600}}>| 👤 {personel}</span>}</div>
                {a.cevaplar&&aktifRol!=="sekreter"&&(
                  <details style={{marginTop:6}}>
                    <summary style={{cursor:"pointer",fontSize:12,color:"#6366f1"}}>Cevapları göster</summary>
                    <div style={{marginTop:6,padding:"8px 10px",background:"#f7f7f5",borderRadius:8}}>
                      {Object.entries(a.cevaplar).map(([k,v])=>(<div key={k} style={{marginBottom:4}}><strong style={{color:"#555"}}>{soruMetni(a.anket_tipi,k)}</strong><br/><span style={{color:"#333"}}>{String(v)}</span></div>))}
                    </div>
                  </details>
                )}
              </div>);
      ...
      {/* ═══ HASTA GÖRÜŞLERİ SEKMESİ ═══ */}
      {anketSekme==="gorusler"&&(
        <div>
          {yorumlar.length===0?(
            <div style={{background:"#fff",border:"1px solid #e8e6e0",borderRadius:12,padding:"3rem",textAlign:"center",color:"#aaa"}}>Henüz görüş yazan hasta yok.</div>
          ):(
            <div style={{display:"flex",flexDirection:"column",gap:8}}>
              {yorumlar.map((a,i)=>{
                const metin=a.anket_tipi==="cilt"?a.cevaplar?.s9:a.cevaplar?.s10;
                ...
                const personel=a.oda==="alex"&&randevuSaati?alexPersoneliTahmini(a.randevu_tarih,randevuSaati):a.oda==="soprano"?"Hanife":null;
                return(
                  <div key={a.id||i} style={{background:"#fff",border:"1px solid #e8e6e0",borderRadius:12,padding:"12px 16px"}}>
                    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
                    ...
                    {metin&&metin.trim()&&<div style={{fontSize:13,color:"#555",background:"#f7f7f5",padding:"10px 12px",borderRadius:8,marginBottom:ekYorum?6:0,lineHeight:1.5}}>"{metin.trim()}"</div>}
                    {ekYorum&&ekYorum.trim()&&<div style={{fontSize:13,color:"#dc2626",background:"#fef2f2",padding:"10px 12px",borderRadius:8,lineHeight:1.5}}>📝 Ek görüş: "{ekYorum.trim()}"</div>}
                  </div>
                );
              })}
            </div>
...
function GelmeyenlerSekme({randevular,aktifRol,onDurumGuncelle}){
  ...
  // Tüm "Gelmedi" randevular — sadece takip başlangıç tarihinden itibaren (öncesi güvenilir değil)
  const gelmeyenler=randevular.filter(r=>r.durum==="Gelmedi"&&r.tarih>=GELMEDI_TAKIP_BASLANGIC).sort((a,b)=>b.tarih.localeCompare(a.tarih));
...
// ── BEKLEYEN HASTA TAKİP ─────────────────────────────────────────────────────
// Alex/Soprano: randevular tablosundan otomatik. Dr İşlem: sekreter her sabah doktor takviminden elle girer.
// Geliş bilgisi hasta_gelis tablosunda tutulur; Supabase Realtime ile tüm ekranlara anında yansır.
// Realtime bağlantısı koparsa 20 sn'de bir sadece bugünün küçük hasta_gelis listesini çeker.
function suanSaatTR(){
  return new Intl.DateTimeFormat("tr-TR",{timeZone:"Europe/Istanbul",hour:"2-digit",minute:"2-digit",hour12:false}).format(new Date());
}
function bhNormalize(s){return (s||"").toString().trim().toLocaleLowerCase("tr").replace(/[ıi̇]/g,"i").replace(/ş/g,"s").replace(/ğ/g,"g").replace(/ü/g,"u").replace(/ö/g,"o").replace(/ç/g,"c");}
function bhSaatDuzelt(s){
  const m=(s||"").toString().trim().match(/^(\d{1,2})[:.](\d{2})/);
  if(!m)return null;
  const h=Number(m[1]),dk=Number(m[2]);
  if(h>23||dk>59)return null;
  return `${String(h).padStart(2,"0")}:${String(dk).padStart(2,"0")}`;
}
function bhTarihDuzelt(s){
  const t=(s||"").toString().trim();
  if(/^\d{4}-\d{2}-\d{2}$/.test(t))return t;
  const m=t.match(/^(\d{1,2})[./](\d{1,2})[./](\d{4})$/);
  return m?`${m[3]}-${m[2].padStart(2,"0")}-${m[1].padStart(2,"0")}`:null;
}
// .xlsx = zip. Ek kütüphane olmadan tarayıcının kendi DecompressionStream'i ile açılır.
async function bhZipOku(buf){
  ...
  const adet=dv.getUint16(eocd+10,true);let p=dv.getUint32(eocd+16,true);
  const cikti={};
  for(let k=0;k<adet;k++){
    if(dv.getUint32(p,true)!==0x02014b50)break;
    const yontem=dv.getUint16(p+10,true),boyut=dv.getUint32(p+20,true),adLen=dv.getUint16(p+28,true),ekLen=dv.getUint16(p+30,true),yorumLen=dv.getUint16(p+32,true),yerel=dv.getUint32(p+42,true);
    const ad=new TextDecoder().decode(new Uint8Array(buf,p+46,adLen));
    p+=46+adLen+ekLen+yorumLen;
    const veri=new Uint8Array(buf,yerel+30+dv.getUint16(yerel+26,true)+dv.getUint16(yerel+28,true),boyut);
    if(yontem===0)cikti[ad]=veri;
    else if(yontem===8){const akis=new Blob([veri]).stream().pipeThrough(new DecompressionStream("deflate-raw"));cikti[ad]=new Uint8Array(await new Response(akis).arrayBuffer());}
  }
  return cikti;
}
async function bhXlsxSatirlar(file){
  const dosyalar=await bhZipOku(await file.arrayBuffer());
  const xml=a=>dosyalar[a]?new DOMParser().parseFromString(new TextDecoder("utf-8").decode(dosyalar[a]),"application/xml"):null;
  const ss=xml("xl/sharedStrings.xml");
  const paylasilan=ss?[...ss.getElementsByTagName("si")].map(si=>[...si.getElementsByTagName("t")].map(t=>t.textContent).join("")):[];
  const sayfa=Object.keys(dosyalar).filter(k=>/^xl\/worksheets\/sheet\d+\.xml$/.test(k)).sort()[0];
  const sh=xml(sayfa);if(!sh)throw new Error("Excel sayfası bulunamadı");
  const satirlar=[];
  for(const row of sh.getElementsByTagName("row")){
    const h=[];
    for(const c of row.getElementsByTagName("c")){
      let col=0;for(const ch of (c.getAttribute("r")||"").replace(/\d+/g,""))col=col*26+(ch.charCodeAt(0)-64);
      col=col>0?col-1:h.length;
      const t=c.getAttribute("t"),v=c.getElementsByTagName("v")[0]?.textContent??"";
      h[col]=t==="s"?(paylasilan[Number(v)]??""):t==="inlineStr"?[...c.getElementsByTagName("t")].map(x=>x.textContent).join(""):v;
    }
    satirlar.push(Array.from(h,x=>x??""));
  }
  return satirlar;
}
// DoktorTakvimi "randevulara genel bakış" Excel'i: Tarih | Hasta/Danışan | Hizmetler | Tutar | Kaynak | Durum
// Dosyada saat yok; ileride saatli gelirse (tarih+saat hücresi veya "Saat" kolonu) otomatik okunur.
function bhDoktorTakvimiOku(satirlar){
  const bi=satirlar.findIndex(r=>r.some(x=>bhNormalize(x).startsWith("hasta")));
  if(bi<0)throw new Error("'Hasta/Danışan' başlığı bulunamadı. DoktorTakvimi 'randevulara genel bakış' dosyası mı?");
  const b=satirlar[bi].map(bhNormalize);
  const idx=(...a)=>b.findIndex(x=>a.some(y=>x.startsWith(y)));
  const iT=idx("tarih"),iH=idx("hasta"),iI=idx("hizmet","islem"),iD=idx("durum"),iS=idx("saat","baslangic");
  const kayitlar=[];
  satirlar.slice(bi+1).forEach(r=>{
    const hasta=(r[iH]||"").toString().trim().replace(/\s+/g," ");if(!hasta)return;
    const ham=(iT>=0?r[iT]:"").toString().trim();
    let tarih=null,saat=null;
    if(/^\d+(\.\d+)?$/.test(ham)&&Number(ham)>30000){
      const n=Number(ham);tarih=new Date(Math.round((n-25569)*86400000)).toISOString().slice(0,10);
      const dk=Math.round((n%1)*1440);if(dk>0)saat=minToTime(dk);
    }else{const [d,sa]=ham.split(/\s+/);tarih=bhTarihDuzelt(d);saat=bhSaatDuzelt(sa||"");}
    if(!saat&&iS>=0)saat=bhSaatDuzelt(r[iS]);
    const islem=iI>=0?(r[iI]||"").toString().trim():"";
    kayitlar.push({tarih,saat:saat||"",hasta:hasta.toLocaleUpperCase("tr"),islem:bhNormalize(islem)==="randevu"?"":islem,tel:"",notlar:"",durum:iD>=0?(r[iD]||"").toString().trim():"",sure:""});
  });
  return kayitlar;
}
// DoktorTakvimi "Randevu listesi" yazdırma sayfası → "PDF olarak kaydet" çıktısı.
// Kolonlar: Saat | Hasta / Danışan | Telefon | Sigorta | Hizmet | Notlar  (hücreler ortalı, bazıları 2 satır)
// pdf.js ilk kullanımda CDN'den yüklenir — package.json değişmez.
const BH_PDFJS="https://cdn.jsdelivr.net/npm/pdfjs-dist@4.10.38/build/";
let bhPdfjsSozu=null;
function bhPdfjsYukle(){
  if(!bhPdfjsSozu){
    bhPdfjsSozu=import(/* @vite-ignore */ BH_PDFJS+"pdf.min.mjs").then(m=>{m.GlobalWorkerOptions.workerSrc=BH_PDFJS+"pdf.worker.min.mjs";return m;})
      .catch(e=>{bhPdfjsSozu=null;throw new Error("PDF okuyucu yüklenemedi (internet bağlantısı?)");});
  }
  return bhPdfjsSozu;
}
async function bhDoktorTakvimiPdfOku(file){
  const pdfjs=await bhPdfjsYukle();
  const doc=await pdfjs.getDocument({data:new Uint8Array(await file.arrayBuffer())}).promise;
  let tarih=null,sinirlar=null;
  const kayitlar=[];
  for(let sn=1;sn<=doc.numPages;sn++){
    const sayfa=await doc.getPage(sn);
    const ic=await sayfa.getTextContent();
    // 1) Aynı satırda bitişik parçaları birleştir (kelime kelime gelen metinler tek hücre olsun)
    const ogeler=ic.items.filter(it=>it.str&&it.str.trim()).map(it=>({x:it.transform[4],y:it.transform[5],w:it.width,s:it.str}))
      .sort((a,b)=>Math.abs(b.y-a.y)>2?b.y-a.y:a.x-b.x);
    const parcalar=[];
    ogeler.forEach(o=>{
      const son=parcalar[parcalar.length-1];
      if(son&&Math.abs(son.y-o.y)<=2&&o.x-(son.x+son.w)<6){son.s+=(o.x-(son.x+son.w)>0.8&&!son.s.endsWith(" ")&&!o.s.startsWith(" ")?" ":"")+o.s;son.w=o.x+o.w-son.x;}
      else parcalar.push({...o});
    });
    parcalar.forEach(p=>{p.s=p.s.replace(/\s+/g," ").trim();p.cx=p.x+p.w/2;});
    if(!tarih){const t=parcalar.map(p=>p.s.match(/(\d{2})[./](\d{2})[./](\d{4})/)&&/randevu/i.test(p.s)?p.s.match(/(\d{2})[./](\d{2})[./](\d{4})/):null).find(Boolean);if(t)tarih=`${t[3]}-${t[2]}-${t[1]}`;}
    // 2) Kolon sınırları: başlık merkezlerinin orta noktaları (başlık sadece 1. sayfada)
    if(!sinirlar){
      const bul=re=>parcalar.find(p=>re.test(bhNormalize(p.s)));
      const b=[bul(/^saat$/),bul(/^hasta/),bul(/^telefon$/),bul(/^sigorta$/),bul(/^hizmet/),bul(/^not/)];
      if(b.some(x=>!x))continue;
      const bY=b[0].y;
      const m=b.map(x=>x.cx);
      sinirlar={y:bY,orta:m.slice(0,-1).map((c,i)=>(c+m[i+1])/2)};
      parcalar.splice(0,parcalar.length,...parcalar.filter(p=>p.y<bY-2));
    }
    const kolon=p=>{let i=0;while(i<sinirlar.orta.length&&p.cx>sinirlar.orta[i])i++;return i;};// 0 saat,1 hasta,2 tel,3 sigorta,4 hizmet,5 not
    // 3) Satırlar: saat kolonundaki SS:DD parçaları; diğer parçalar en yakın saat satırına
    const satirlar=parcalar.filter(p=>kolon(p)===0&&/^\d{1,2}:\d{2}$/.test(p.s)).map(p=>({y:p.y,saat:bhSaatDuzelt(p.s),h:[[],[],[],[],[],[]]}));
    if(!satirlar.length)continue;
    parcalar.forEach(p=>{
      const k=kolon(p);if(k===0)return;
      let en=null,fark=1e9;satirlar.forEach(r=>{const d=Math.abs(r.y-p.y);if(d<fark){fark=d;en=r;}});
      if(en&&fark<=18)en.h[k].push(p);
    });
    satirlar.forEach(r=>{
      const birlestir=(k,ayrac=" ")=>r.h[k].sort((a,b)=>b.y-a.y).map(p=>p.s).join(ayrac).trim();
      const hasta=birlestir(1);if(!hasta)return;
      const hizmet=birlestir(4);
      // İsim olduğu gibi alınır: DoktorTakvimi bu listede büyük "I"yı "i" yapıyor (YILDIRIM → Yildirim),
      // tekrar büyütmek YİLDİRİM gibi hatalı sonuç verir.
      kayitlar.push({tarih,saat:r.saat||"",hasta,tel:birlestir(2,"").replace(/\s/g,""),islem:bhNormalize(hizmet)==="randevu"?"":hizmet,notlar:birlestir(5),durum:"",sure:""});
    });
  }
  if(!sinirlar)throw new Error("Başlık satırı (Saat / Hasta / Telefon / Hizmet) bulunamadı. DoktorTakvimi 'Randevu listesi' çıktısı mı?");
  return kayitlar.map(k=>({...k,tarih:k.tarih||tarih}));
}
const BH_KREM_VARSAYILAN=30;
// Uyuşma kuralları (klinik listesi). erken:true → hasta krem için randevudan bu kadar önce gelir.
// Sıra önemli: ilk eşleşen kural geçerli. null = krem yok.
// Uyuşma hedef süreleri (klinik listesi). Kremi sürme kararı doktorda; bu tablo sadece sayacın hedefini verir.
// Sıra önemli: ilk eşleşen geçerli. null = hedef yok (tuş yine basılabilir, sayaç hedefsiz sayar).
const BH_KREM_KURALLARI=[
  [/kontrol/,null],
  [/botoks/,s=>/koltuk|\bel\b|\beller\b/.test(s)?{sure:20}:null],
  [/altin ?igne|morpheus/,{sure:45}],
  [/fraxis|fraksiyonel/,{sure:45}],
  [/mezo|matrix|lifting|somon|dna|pdrn|\basi(si)?\b/,s=>/goz/.test(s)?{sure:15}:{sure:45}],
  [/dudak.*dolgu|dolgu.*dudak/,{sure:15}],
];
function bhKremKurali(islem){
  const n=bhNormalize(islem);
  for(const [re,sonuc] of BH_KREM_KURALLARI){if(re.test(n))return typeof sonuc==="function"?sonuc(n):sonuc;}
  return null;
}
const BH_KREM_SURELER=[15,20,30,40,45,60];
const BH_SURELER=[10,15,20,30,45,60,90];

const BH_ODALAR=[["alex","🟢 Alex"],["soprano","🟣 Soprano"],["dr","🩺 Dr İşlem"],["tumu","📋 Tümü"]];
const BH_ODA_ETIKET={alex:["Alex","#166534","#dcfce7"],soprano:["Soprano","#5b21b6","#ede9fe"],dr:["Dr","#0f766e","#ccfbf1"]};
const BhOdaRozet=({oda})=>{const e=BH_ODA_ETIKET[oda];return e?<span style={{fontSize:10,fontWeight:800,color:e[1],background:e[2],padding:"1px 6px",borderRadius:4,marginRight:6,verticalAlign:"middle",whiteSpace:"nowrap"}}>{e[0]}</span>:null;};

function BekleyenHastaSekme({randevular,aktifRol,aktifKullanici,showToast}){
  const [tarih,setTarih]=useState(today());
  const [gelisler,setGelisler]=useState([]);
  const [yukleniyor,setYukleniyor]=useState(true);
  const [canli,setCanli]=useState(false);
  const [tamEkran,setTamEkran]=useState(false);
  const [saatSimdi,setSaatSimdi]=useState(suanSaatTR());
  ...
  // ── Veri: hasta_gelis (seçili gün) ──
  const gelisleriYukle=useCallback(async()=>{
    try{
      const d=await sbGet("hasta_gelis",`tarih=eq.${tarih}`);
      setGelisler(d);
    }catch(e){showToast("Geliş listesi yüklenemedi: "+String(e.message).slice(0,120),"error");}
    finally{setYukleniyor(false);}
  },[tarih]);
  useEffect(()=>{setYukleniyor(true);gelisleriYukle();},[gelisleriYukle]);

  // ── Realtime (Supabase Phoenix websocket, kütüphanesiz) ──
  useEffect(()=>{
    let ws=null,hb=null,yenidenTimer=null,kapandi=false,ref=1,deneme=0;
    const topic=`realtime:hasta_gelis_${tarih}`;
    ...
    function baglan(){
      ...
      ws.onopen=()=>{
        gonder({topic,event:"phx_join",payload:{config:{broadcast:{self:false},presence:{key:""},postgres_changes:[{event:"*",schema:"public",table:"hasta_gelis",filter:`tarih=eq.${tarih}`}]},access_token:CURRENT_TOKEN||SB_KEY}});
        let say=0;
        hb=setInterval(()=>{
          gonder({topic:"phoenix",event:"heartbeat",payload:{}});
          // 45 dk'da bir yenilenen oturum anahtarını açık kanala da ilet (5 dk'da bir), yoksa kanal 1 saat sonra düşer
          if(++say%12===0&&CURRENT_TOKEN)gonder({topic,event:"access_token",payload:{access_token:CURRENT_TOKEN}});
        },25000);
      };
      ws.onmessage=ev=>{
        let m;try{m=JSON.parse(ev.data);}catch{return;}
        if(m.topic!==topic)return;
        if(m.event==="phx_reply"&&m.payload?.status==="ok"&&m.payload?.response?.postgres_changes){setCanli(true);deneme=0;gelisleriYukle();}
        // Kanal hata verir/kapanırsa bağlantıyı kapat → otomatik yeniden bağlanır (güncel token ile)
        if((m.event==="phx_reply"&&m.payload?.status==="error")||(m.event==="system"&&m.payload?.status==="error")||m.event==="phx_close"||m.event==="phx_error"){setCanli(false);try{ws.close();}catch{}}
        if(m.event==="postgres_changes"){
          const d=m.payload?.data;if(!d)return;
          if(d.type==="DELETE"){const id=d.old_record?.id;setGelisler(g=>g.filter(x=>x.id!==id));}
          else if(d.record){const rec=d.record;setGelisler(g=>{const var_=g.some(x=>x.id===rec.id);return var_?g.map(x=>x.id===rec.id?rec:x):[...g,rec];});}
        }
      };
      ws.onclose=()=>{setCanli(false);clearInterval(hb);if(!kapandi)planla();};
  ...
  },[tarih,gelisleriYukle]);

  // Yedek: realtime bağlı değilken 20 sn'de bir sadece bu küçük tabloyu tazele
  useEffect(()=>{
    if(canli)return;
    const t=setInterval(()=>{if(document.visibilityState==="visible")gelisleriYukle();},20000);
    return()=>clearInterval(t);
  },[canli,gelisleriYukle]);

  // ── Listeler ──
  const gelisByRandevu={};gelisler.forEach(g=>{if(g.randevu_id)gelisByRandevu[g.randevu_id]=g;});
  // Randevu dışı satırlar (Dr İşlem kayıtları + randevusuz gelen hastalar) hasta_gelis tablosundan gelir
  // Randevu dışı satırlar (Dr İşlem kayıtları + randevusuz gelen hastalar) hasta_gelis tablosundan gelir
  const gelisSatiri=g=>({key:"d"+g.id,oda:g.oda,randevuId:null,gelisId:g.id,saat:g.saat,sure:g.sure||0,hasta:g.hasta,tel:g.tel||"",islem:[g.islem,g.sure?g.sure+" dk":""].filter(Boolean).join(" · "),notlar:g.notlar||"",randevuDurum:null,
    geldi:!!g.geldi,gelisSaati:g.gelis_saati||"",alinmaSaati:g.alinma_saati||"",iptalSaati:g.iptal_saati||"",kaydeden:g.kaydeden||"",
    uyusmaSaati:g.uyusma_saati||"",uyusmaHedef:g.uyusma_saati?(g.uyusma_hedef||0):(g.oda==="dr"?(bhKremKurali(g.islem)?.sure||0):0),
    randevusuz:g.kaynak==="randevusuz"});
  function randevuSatirlari(oda){
    return randevular.filter(r=>r.oda===oda&&r.tarih===tarih)
      .map(r=>{const g=gelisByRandevu[r.id];return {key:"r"+r.id,oda,randevuId:r.id,gelisId:g?.id,saat:r.saat,sure:r.sure||0,tel:r.tel||"",hasta:r.hasta,islem:(r.bolgeler||[]).join(", "),notlar:r.notlar||"",randevuDurum:r.durum,geldi:!!g?.geldi,gelisSaati:g?.gelis_saati||"",alinmaSaati:g?.alinma_saati||"",iptalSaati:g?.iptal_saati||"",kaydeden:g?.kaydeden||""};})
  ...
  function siralaDegistir(v){setSiralama(v);try{window.localStorage.setItem("kl_bh_sira",v);}catch{}}
  // Geliş sırası: kağıt listedeki gibi — gelenler geliş saatine göre üstte, gelmeyenler randevu saatine göre altta
  const sirala=L=>[...L].sort((a,b)=>{
    if(siralama==="gelis"){
      if(a.geldi!==b.geldi)return a.geldi?-1:1;
      if(a.geldi&&b.geldi)return timeToMin(a.gelisSaati||a.saat)-timeToMin(b.gelisSaati||b.saat);
    }
    return timeToMin(a.saat)-timeToMin(b.saat);
  });
  const listeler={alex:sirala(randevuSatirlari("alex")),soprano:sirala(randevuSatirlari("soprano")),dr:sirala(gelisler.filter(g=>g.oda==="dr").map(gelisSatiri))};
  listeler.tumu=sirala([...listeler.alex,...listeler.soprano,...listeler.dr]);
  const satirlar=listeler[aktifOda]||[];
  const bugunMu=tarih===today();
  const simdiDk=timeToMin(saatSimdi);

  // ── Krem / uyuşma (sadece Dr İşlem) ──
  // Kremi süren "Krem sürüldü"ye basar → krem saati + ⏳ sayaç. Sayaç "Alındı"ya basılana kadar sayar.
  function kremDurum(s){
    if(!s.uyusmaSaati)return null;
    const bitis=s.alinmaSaati?timeToMin(s.alinmaSaati):(bugunMu?simdiDk:null);
  ...
  const kremUygun=s=>s.oda==="dr"&&(!!s.uyusmaSaati||s.uyusmaHedef>0);
  ...
  async function kremIsaretle(s,sur){
    ...
    if(!sur&&!window.confirm(`${s.hasta} için krem kaydı silinsin mi?`))return;
    setIslemde(s.key);
    const saat=suanSaatTR();
    const deg=sur?{uyusma_saati:saat,uyusma_hedef:s.uyusmaHedef||null}:{uyusma_saati:null,uyusma_hedef:null,alinma_saati:null};
    if(await gelisYaz(s,deg)&&sur)showToast(`💧 ${s.hasta} — krem ${saat}`);
    setIslemde(null);
  }
  function KremGosterge({s}){
    const k=kremDurum(s);
    if(!k){
      if(kremTusuGorunur(s))return <button onClick={e=>{e.stopPropagation();kremIsaretle(s,true);}} disabled={islemde===s.key}
        style={{...btnPrimary,background:"#0d9488",padding:"6px 12px",fontSize:13*fs,fontWeight:700,whiteSpace:"nowrap"}}>💧 Krem sürüldü</button>;
      return <span style={{color:"#bbb"}}>—</span>;
    }
    const yuzde=k.hedef&&k.gecen!==null?Math.min(100,Math.round(k.gecen/k.hedef*100)):0;
    if(k.durdu)return <span style={{fontWeight:700,color:"#4b5563",whiteSpace:"nowrap"}}>{k.saat} · {sayacMetni(k)}</span>;
    return(
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        <div>
          {k.hazir
            ?<span style={{fontSize:13*fs,fontWeight:800,color:"#fff",background:"#0d9488",padding:"3px 10px",borderRadius:20,whiteSpace:"nowrap"}}>{k.saat} · {sayacMetni(k)}</span>
            :<span style={{fontSize:14*fs,fontWeight:800,color:"#0f766e",whiteSpace:"nowrap"}}>{k.saat} · {sayacMetni(k)}</span>}
          {!k.hazir&&k.hedef>0&&<div style={{height:4,width:120,background:"#ccfbf1",borderRadius:4,overflow:"hidden",marginTop:3}}><div style={{width:yuzde+"%",height:"100%",background:"#14b8a6"}}/></div>}
        </div>
        {!tamEkran&&drDuzenleyebilir&&<button onClick={()=>kremIsaretle(s,false)} title="Krem kaydını sil" style={{background:"none",border:"none",color:"#bbb",cursor:"pointer",fontSize:12,padding:0}}>✕</button>}
      </div>
    );
  }
  ...
  function satirDurumu(s){
    ...
    return {etiket:"",metin:"",renk:"#555",bg:"transparent",satirBg:"#fff"};
  }

  // ── İşlemler ──
  async function geldiIsaretle(s,geldi){
    if(!gelisIsaretleyebilir||islemde)return;
    if(!geldi&&!window.confirm(`${s.hasta} için "Geldi" kaydı geri alınsın mı?`))return;
    setIslemde(s.key);
    const saat=suanSaatTR();
    const kaydeden=login||ROLLER[aktifRol];
    // İyimser güncelleme — satır hemen yeşile döner
    const onceki=gelisler;
    try{
      if(s.gelisId){
        setGelisler(g=>g.map(x=>x.id===s.gelisId?{...x,geldi,gelis_saati:geldi?saat:null,kaydeden}:x));
        await sbUpdate("hasta_gelis",s.gelisId,{geldi,gelis_saati:geldi?saat:null,kaydeden,guncelleme:new Date().toISOString()});
      }else{
        const gecici={id:-Date.now(),tarih,oda:s.oda,randevu_id:s.randevuId,saat:s.saat,hasta:s.hasta,islem:s.islem,geldi,gelis_saati:geldi?saat:null,kaydeden,kaynak:"randevu"};
        setGelisler(g=>[...g,gecici]);
        const r=await fetch(`${SB_URL}/rest/v1/hasta_gelis?on_conflict=randevu_id`,{method:"POST",headers:{...HDR(),"Prefer":"return=representation,resolution=merge-duplicates"},
          body:JSON.stringify({tarih,oda:s.oda,randevu_id:s.randevuId,saat:s.saat,hasta:s.hasta,islem:s.islem,geldi,gelis_saati:geldi?saat:null,kaydeden,kaynak:"randevu",guncelleme:new Date().toISOString()})});
        if(!r.ok)throw new Error(await r.text());
        const [kayit]=await r.json();
        setGelisler(g=>{const temiz=g.filter(x=>x.id!==gecici.id&&x.id!==kayit.id);return [...temiz,kayit];});
      }
      if(geldi)showToast(`✅ ${s.hasta} geldi — ${saat}`);
    }catch(e){
      setGelisler(onceki);
      showToast("Kaydedilemedi: "+String(e.message).slice(0,140),"error");
    }finally{setIslemde(null);}
  }

  // hasta_gelis'e yaz: satır varsa güncelle, randevu satırı ise yoksa oluştur (iyimser)
  async function gelisYaz(s,deg){
    const onceki=gelisler;
    try{
      if(s.gelisId>0){
        setGelisler(g=>g.map(x=>x.id===s.gelisId?{...x,...deg}:x));
        await sbUpdate("hasta_gelis",s.gelisId,{...deg,guncelleme:new Date().toISOString()});
      }else if(s.randevuId){
        const govde={tarih,oda:s.oda,randevu_id:s.randevuId,saat:s.saat,hasta:s.hasta,islem:s.islem,kaynak:"randevu",...deg};
        const gecici={id:-Date.now(),...govde};
        setGelisler(g=>[...g,gecici]);
        const r=await fetch(`${SB_URL}/rest/v1/hasta_gelis?on_conflict=randevu_id`,{method:"POST",headers:{...HDR(),"Prefer":"return=representation,resolution=merge-duplicates"},body:JSON.stringify({...govde,guncelleme:new Date().toISOString()})});
        if(!r.ok)throw new Error(await r.text());
        const [kayit]=await r.json();
        setGelisler(g=>[...g.filter(x=>x.id!==gecici.id&&x.id!==kayit.id),kayit]);
      }
      return true;
  ...
  async function alindiIsaretle(s,alindi){
    ...
    setIslemde(s.key);
    const saat=suanSaatTR();
    const deg={alinma_saati:alindi?saat:null};
    if(await gelisYaz(s,deg)&&alindi)showToast(`▶ ${s.hasta} alındı — ${saat}`);
    setIslemde(null);
  }
  ...
  async function randevusuzEkle(e){
    e.preventDefault();
    const hasta=randevusuz.hasta.trim().replace(/\s+/g," ");
    if(!hasta){showToast("Hasta adı gerekli","error");return;}
    setEkleniyor(true);
    try{
      const saat=suanSaatTR();
      const [k]=await sbInsert("hasta_gelis",[{tarih,oda:aktifOda,saat,hasta:aktifOda==="dr"?hasta:hasta.toLocaleUpperCase("tr"),islem:randevusuz.islem.trim(),geldi:true,gelis_saati:saat,kaydeden:login||ROLLER[aktifRol],kaynak:"randevusuz"}]);
      setGelisler(g=>g.some(x=>x.id===k.id)?g:[...g,k]);
      showToast(`${hasta} eklendi (randevusuz, geldi ${saat})`);
      setRandevusuz({acik:false,hasta:"",islem:""});
    }catch(err){showToast("Eklenemedi: "+String(err.message).slice(0,140),"error");}
    finally{setEkleniyor(false);}
  }

  // Seçili günün tüm Dr İşlem listesini sil (yanlış/eski PDF yüklendiğinde baştan yüklemek için)
  async function drListeSil(){
    if(!gelisIsaretleyebilir&&aktifRol!=="personel")return;
    const L=gelisler.filter(g=>g.oda==="dr"&&g.tarih===tarih&&g.id>0);
    if(!L.length){showToast("Silinecek Dr İşlem kaydı yok","error");return;}
    const islenmis=L.filter(g=>g.geldi||g.uyusma_saati||g.iptal_saati).length;
    if(!window.confirm(`${formatDate(tarih)} — Dr İşlem listesindeki ${L.length} hastanın TAMAMI silinsin mi?`))return;
    if(islenmis&&!window.confirm(`Dikkat: ${islenmis} hastada Geldi / krem / iptal kaydı var, bunlar da silinecek. Emin misiniz?`))return;
    try{
      const r=await fetch(`${SB_URL}/rest/v1/hasta_gelis?oda=eq.dr&tarih=eq.${tarih}`,{method:"DELETE",headers:{...HDR(),"Prefer":"return=minimal"}});
      if(!r.ok)throw new Error(await r.text());
      setGelisler(g=>g.filter(x=>!(x.oda==="dr"&&x.tarih===tarih)));
      showToast(`Dr İşlem listesi temizlendi (${L.length} kayıt)`);
    }catch(e){showToast("Silinemedi: "+String(e.message).slice(0,140),"error");}
  }

  async function drSil(s){
    if(!drDuzenleyebilir||!s.gelisId)return;
    if(!window.confirm(`${s.saat} ${s.hasta} listeden silinsin mi?`))return;
    try{await sbDelete("hasta_gelis",s.gelisId);setGelisler(g=>g.filter(x=>x.id!==s.gelisId));}
    catch(e){showToast("Silinemedi: "+String(e.message).slice(0,120),"error");}
  }

  async function drKaydet(kayitlar){
    // Aynı gün + saat + hasta zaten varsa atla
    const mevcut=new Set(gelisler.filter(g=>g.oda==="dr").map(g=>`${g.tarih}|${g.saat}|${bhNormalize(g.hasta)}`));
    const yeni=kayitlar.filter(k=>!mevcut.has(`${k.tarih}|${k.saat}|${bhNormalize(k.hasta)}`));
    if(yeni.length===0){showToast("Bu saatte bu hasta zaten listede.","error");return 0;}
    const kaydeden=login||ROLLER[aktifRol];
    const eklenen=await sbInsert("hasta_gelis",yeni.map(k=>({...k,oda:"dr",geldi:false,kaynak:"manuel",kaydeden})));
    setGelisler(g=>[...g,...eklenen.filter(e=>e.tarih===tarih&&!g.some(x=>x.id===e.id))]);
    return eklenen.length;
  }

  async function manuelEkle(e){
    e.preventDefault();
    const saat=bhSaatDuzelt(manuel.saat);
    const hasta=manuel.hasta.trim().replace(/\s+/g," ");
    if(!saat||!hasta){showToast("Saat ve hasta adı gerekli","error");return;}
    setEkleniyor(true);
    try{
      const n=await drKaydet([{tarih,saat,hasta:hasta.toLocaleUpperCase("tr"),islem:manuel.islem.trim(),notlar:manuel.notlar.trim(),sure:manuel.sure?Number(manuel.sure):null}]);
      if(n){showToast(`${saat} ${hasta.toLocaleUpperCase("tr")} eklendi`);setManuel({saat:"",hasta:"",islem:"",notlar:"",sure:""});setTimeout(()=>saatInputRef.current?.focus(),0);}
    }catch(err){showToast("Eklenemedi: "+String(err.message).slice(0,120),"error");}
    finally{setEkleniyor(false);}
  }

  // ── DoktorTakvimi Excel içe aktarma ──
  async function excelSec(e){
    const f=e.target.files?.[0];e.target.value="";
    if(!f)return;
    const pdfMi=/\.pdf$/i.test(f.name)||f.type==="application/pdf";
    if(!pdfMi&&typeof DecompressionStream==="undefined"){showToast("Bu tarayıcı Excel dosyasını açamıyor. Chrome'u güncelleyin.","error");return;}
    try{
      if(pdfMi)showToast("PDF okunuyor…");
      const kayitlar=pdfMi?await bhDoktorTakvimiPdfOku(f):bhDoktorTakvimiOku(await bhXlsxSatirlar(f));
      if(!kayitlar.length){showToast("Dosyada randevu bulunamadı","error");return;}
      const tarihler=[...new Set(kayitlar.map(k=>k.tarih).filter(Boolean))];
      const mevcutlar=tarihler.length?await sbGet("hasta_gelis",`oda=eq.dr&tarih=in.(${tarihler.join(",")})&select=id,tarih,hasta`):[];
      const mevcutSet=new Set(mevcutlar.map(m=>m.tarih+"|"+bhNormalize(m.hasta)));
      setExcelOnizleme({dosya:f.name,saatli:kayitlar.some(k=>k.saat),satirlar:kayitlar.map((k,i)=>{
        const iptal=/iptal|gelmedi|ertelen/.test(bhNormalize(k.durum));
        const mevcut=mevcutSet.has(k.tarih+"|"+bhNormalize(k.hasta));
        return {...k,id:i,iptal,mevcut,sec:!iptal&&!mevcut&&!!k.tarih};
      })});
    }catch(err){showToast("Dosya okunamadı: "+String(err.message).slice(0,160),"error");}
  }
  function excelSatirGuncelle(id,alan,deger){
    setExcelOnizleme(o=>({...o,satirlar:o.satirlar.map(s=>s.id===id?{...s,[alan]:deger}:s)}));
  }
  async function excelKaydet(){
    const secili=excelOnizleme.satirlar.filter(s=>s.sec);
    if(!secili.length){showToast("Eklenecek hasta seçilmedi","error");return;}
    const eksik=secili.filter(s=>!bhSaatDuzelt(s.saat));
    if(eksik.length){showToast(`${eksik.length} hastanın saati girilmedi: ${eksik.slice(0,3).map(s=>s.hasta).join(", ")}${eksik.length>3?"…":""}`,"error");return;}
    setEkleniyor(true);
    try{
      const kaydeden=login||ROLLER[aktifRol];
      const eklenen=await sbInsert("hasta_gelis",secili.map(s=>({tarih:s.tarih,saat:bhSaatDuzelt(s.saat),hasta:s.hasta.trim().replace(/\s+/g," "),islem:s.islem.trim(),notlar:(s.notlar||"").trim(),tel:s.tel||null,sure:s.sure?Number(s.sure):null,oda:"dr",geldi:false,kaynak:"doktortakvimi",kaydeden})));
      setGelisler(g=>[...g,...eklenen.filter(x=>x.tarih===tarih&&!g.some(y=>y.id===x.id))]);
      const t=secili[0].tarih;if(t!==tarih)setTarih(t);
      showToast(`${eklenen.length} Dr randevusu eklendi`);
      setExcelOnizleme(null);
    }catch(err){showToast("Kaydedilemedi: "+String(err.message).slice(0,160),"error");}
    finally{setEkleniyor(false);}
  }

  // ── Yazdırılabilir liste (ilk günler kağıtla çift kontrol için) ──
  function listeYazdir(tip){
    ...
    const html=`<!doctype html><html lang="tr"><head><meta charset="utf-8"><title>${tip==="dr"?"Dr İşlem":"Alex-Soprano"} ${esc(tarih)}</title><style>
      ...
      h1{font-size:17px;margin:0 0 2px}.alt{font-size:11px;color:#555;margin-bottom:10px}
      h2{font-size:15px;margin:16px 0 6px;border-bottom:2px solid #000;padding-bottom:3px}h2 span{font-weight:normal;font-size:12px}
      table{width:100%;border-collapse:collapse;font-size:12px}th,td{border:1px solid #444;padding:6px 5px;text-align:left;vertical-align:middle}
      th{background:#eee;font-size:11px}tr{page-break-inside:avoid}td.c,th.c{width:38px;text-align:center}
      .kutu{display:inline-block;width:16px;height:16px;border:1.5px solid #000;line-height:16px;font-weight:bold}
      .saat{width:42px;font-weight:bold}.ad{font-weight:bold}.tel{width:90px;font-size:11px}.yaz{width:62px}.not{width:22%}
      tr.gm td{color:#888}.bos{color:#777;font-style:italic}
    </style></head><body><h1>${tip==="dr"?"Dr İşlem Hasta Listesi":"Alex / Soprano Hasta Listesi"}</h1>
      <div class="alt">${esc(formatDate(tarih))} · Yazdırma: ${esc(suanSaatTR())}</div>${bolumler.map(tablo).join("")}
      <script>window.onload=function(){setTimeout(function(){window.print();},300);}<\/script></body></html>`;
    const w=window.open("","_blank");
    if(!w){showToast("Yazdırma penceresi açılamadı. Tarayıcının açılır pencere engelini kaldırın.","error");return;}
    w.document.open();w.document.write(html);w.document.close();
  }

  // ── Tam ekran ──
  async function tamEkranAc(){
    setTamEkran(true);
  ...
  useEffect(()=>{
    const fn=()=>{if(!document.fullscreenElement)setTamEkran(false);};
  ...
  // ── MOBİL KART GÖRÜNÜMÜ ──
  if(mobil&&!tamEkran){
    const MOBIL_ODALAR=[["dr","🩺 DR"],["alex","🟢 ALEX"],["soprano","🟣 SOPRANO"],["tumu","📋 TÜMÜ"]];
    const SERIT={"Geldi":"#16a34a","Gecikti":"#ca8a04","Gelmedi":"#dc2626","İptal":"#991b1b","Alındı":"#4b5563","":"#d4d4d0"};
    // Sıradaki: süresi dolmuş kremli hasta (Dr) → gelip bekleyen → saati gelmemiş ilk randevu
    const bekleyenler=satirlar.filter(s=>!s.iptalSaati&&s.randevuDurum!=="Gelmedi"&&!s.alinmaSaati);
    const siradaki=!bugunMu?null:(
      bekleyenler.filter(s=>kremDurum(s)?.hazir).sort((a,b)=>timeToMin(a.uyusmaSaati)-timeToMin(b.uyusmaSaati))[0]
      ||[...bekleyenler].filter(s=>s.geldi).sort((a,b)=>timeToMin(a.saat)-timeToMin(b.saat))[0]
      ||bekleyenler.find(s=>!s.geldi&&timeToMin(s.saat)+10>=simdiDk));
    const Kutu=({l,v,c="#1a1a2e",buyuk})=>(
      <div style={{background:"rgba(0,0,0,0.035)",borderRadius:8,padding:"5px 6px",textAlign:"center"}}>
        <div style={{fontSize:10,fontWeight:700,color:"#888",letterSpacing:0.5}}>{l}</div>
        <div style={{fontSize:buyuk?20:17,fontWeight:700,color:c,fontVariantNumeric:"tabular-nums"}}>{v}</div>
      </div>);
    const Kart=({s,buyuk})=>{
      const d=satirDurumu(s),acik=acikKart===s.key,drKart=kremUygun(s),kd=drKart?kremDurum(s):null;
      const iptal=!!s.iptalSaati,soluk=iptal||s.randevuDurum==="Gelmedi";
      const serit=kd&&!kd.durdu?"#0d9488":SERIT[d.etiket];
      const bg=iptal?"#fef2f2":s.alinmaSaati?"#d1d5db":s.geldi?"#f0fdf4":d.etiket==="Gecikti"?"#fef9c3":"#fff";
      ...
      return(
        <div onClick={()=>setAcikKart(acik?null:s.key)} style={{display:"flex",background:bg,borderRadius:12,border:"1px solid #e8e6e0",overflow:"hidden",marginBottom:8,opacity:soluk?0.65:1,cursor:"pointer"}}>
          ...
          <div style={{flex:1,padding:buyuk?"14px 14px":"11px 12px",minWidth:0}}>
            <div style={{display:"flex",alignItems:"baseline",gap:8}}>
              {aktifOda==="tumu"&&<BhOdaRozet oda={s.oda}/>}
              <span style={{fontSize:buyuk?20:17,fontWeight:700,color:iptal?"#9f1239":"#1a1a2e",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:acik?"normal":"nowrap",textDecoration:iptal?"line-through":"none"}}>{s.hasta}</span>
              {s.randevusuz&&<span style={{fontSize:10,fontWeight:700,color:"#7c3aed",background:"#ede9fe",padding:"1px 5px",borderRadius:4,flexShrink:0}}>Randevusuz</span>}
            </div>
            <div style={{fontSize:14,color:"#555",marginTop:3}}>
              ...
              {(s.islem||"—").split(/,\s*/).join(" · ")}
            </div>
            {/* Saat kutuları: Randevu | Geliş | (Dr) K. saati */}
            <div style={{display:"grid",gridTemplateColumns:drKart?"1fr 1fr 1fr":"1fr 1fr",gap:6,marginTop:8}}>
              <Kutu l="RANDEVU" v={s.randevusuz?"—":s.saat} buyuk={buyuk}/>
              <Kutu l="GELİŞ" v={s.gelisSaati||"—"} c={s.gelisSaati?"#166534":"#bbb"} buyuk={buyuk}/>
              {drKart&&(kd?(
                <div style={{background:kd.durdu?"#9ca3af":kd.hazir?"#0d9488":"#ccfbf1",color:kd.durdu?"#111827":kd.hazir?"#fff":"#0f766e",borderRadius:8,padding:"5px 6px",textAlign:"center"}}>
                  <div style={{fontSize:10,fontWeight:700,letterSpacing:0.5,opacity:0.85}}>K. SAATİ</div>
                  <div style={{fontSize:buyuk?20:17,fontWeight:700,fontVariantNumeric:"tabular-nums"}}>{kd.saat}</div>
                  <div style={{fontSize:13,fontWeight:800,fontVariantNumeric:"tabular-nums"}}>{sayacMetni(kd)}</div>
                  {!kd.durdu&&!kd.hazir&&kd.hedef>0&&kd.gecen!==null&&<div style={{height:3,background:"rgba(13,148,136,0.2)",borderRadius:3,marginTop:3,overflow:"hidden"}}><div style={{width:Math.min(100,Math.round(kd.gecen/kd.hedef*100))+"%",height:"100%",background:"#14b8a6"}}/></div>}
                </div>
              ):kremTusuGorunur(s)?(
                <button onClick={e=>{e.stopPropagation();kremIsaretle(s,true);}} disabled={islemde===s.key}
                  style={{background:"#0d9488",color:"#fff",border:"none",borderRadius:8,padding:"5px 4px",fontFamily:"inherit",cursor:"pointer",fontWeight:700,lineHeight:1.2}}>
                  <div style={{fontSize:10,letterSpacing:0.5,opacity:0.85}}>K. SAATİ</div><div style={{fontSize:14}}>💧 Krem<br/>sürüldü</div>
                </button>
            ...
            <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",gap:8,marginTop:8,minHeight:d.metin||geldiTusu||alindiTusu?40:0}}>
              ...
              <span style={{display:"flex",gap:6,flexShrink:0}}>
                ...
                {alindiTusu&&<button onClick={e=>{e.stopPropagation();alindiIsaretle(s,true);}} disabled={islemde===s.key} style={{...btnPrimary,background:"#4b5563",padding:"10px 16px",fontSize:15,fontWeight:700,borderRadius:10}}>▶ ALINDI</button>}
              </span>
            </div>
            {acik&&(
              <div onClick={e=>e.stopPropagation()} style={{marginTop:10,paddingTop:10,borderTop:"1px dashed #e5e5e0",fontSize:13,color:"#555",display:"flex",flexDirection:"column",gap:6,alignItems:"flex-start"}}>
                {s.tel&&<a href={"tel:"+s.tel} style={{color:"#2563eb",textDecoration:"none"}}>📞 {s.tel}</a>}
                {s.notlar&&<div>📝 {s.notlar}</div>}
                {s.geldi&&s.kaydeden&&<div style={{color:"#999"}}>İşaretleyen: {s.kaydeden}</div>}
    ...
    return(
      <div style={{margin:"-0.5rem -0.75rem"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:10}}>
          <div>
            <div style={{fontSize:13,fontWeight:800,letterSpacing:0.5,color:"#1a1a2e"}}>BEKLEYEN HASTALAR — {bugunMu?"BUGÜN":formatDate(tarih).toLocaleUpperCase("tr")}</div>
            <div style={{fontSize:11,color:canli?"#16a34a":"#b45309",display:"flex",alignItems:"center",gap:4,marginTop:2}}><span style={{width:7,height:7,borderRadius:7,background:canli?"#22c55e":"#f59e0b"}}/>{canli?"Canlı":"Yedek mod"}</div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            {!bugunMu&&<button onClick={()=>setTarih(today())} style={{...navBtnStyle,fontSize:12,padding:"5px 9px"}}>Bugün</button>}
            <span style={{fontSize:20,fontWeight:700,fontVariantNumeric:"tabular-nums",color:"#1a1a2e"}}>{saatSimdi}</span>
          </div>
        </div>
        <div style={{display:"grid",gridTemplateColumns:`repeat(${MOBIL_ODALAR.length},1fr)`,gap:6,marginBottom:12}}>
          {MOBIL_ODALAR.map(([k,l])=>{
            const L=(listeler[k]||[]).filter(x=>!x.iptalSaati),g=L.filter(x=>x.geldi).length,a=aktifOda===k;
            return <button key={k} onClick={()=>{setAktifOda(k);setAcikKart(null);}} style={{padding:"10px 2px",borderRadius:12,border:a?"2px solid #6366f1":"1px solid #ddd",background:a?"#eef0ff":"#fff",color:a?"#4338ca":"#444",fontWeight:800,fontSize:12,cursor:"pointer",fontFamily:"inherit",lineHeight:1.2}}>
              {l}<div style={{fontSize:16,fontWeight:700,marginTop:2,fontVariantNumeric:"tabular-nums"}}>{g}/{L.length}</div>
            </button>;
          })}
        </div>
        <div style={{display:"flex",justifyContent:"flex-end",marginBottom:8}}>
          <button onClick={()=>siralaDegistir(siralama==="gelis"?"randevu":"gelis")} style={{background:"none",border:"none",color:"#6366f1",fontSize:12,fontWeight:600,cursor:"pointer",fontFamily:"inherit",padding:0}}>
            ⇅ {siralama==="gelis"?"Geliş sırası":"Randevu saati"}
          </button>
        </div>
        {siradaki&&(
          <div style={{marginBottom:14}}>
            <div style={{fontSize:11,fontWeight:800,color:"#6366f1",letterSpacing:1,marginBottom:5}}>SIRADAKİ →</div>
            <Kart s={siradaki} buyuk/>
          </div>
        )}
        {yukleniyor&&satirlar.length===0&&<div style={{padding:24,textAlign:"center",color:"#aaa"}}>Yükleniyor…</div>}
        {!yukleniyor&&satirlar.length===0&&<div style={{padding:28,textAlign:"center",color:"#aaa",background:"#fff",borderRadius:12,border:"1px solid #e8e6e0"}}>
          {aktifOda==="dr"?"Bugün için Dr İşlem listesi yok.":aktifOda==="tumu"?"Bugün için hasta yok.":"Bu odada randevu yok."}
        </div>}
        {satirlar.filter(s=>s.key!==siradaki?.key).map(s=><Kart key={s.key} s={s}/>)}
      </div>
    );
  }

  const fs=tamEkran?1.35:1; // tam ekranda yazılar büyür
  const toplam=satirlar.filter(s=>!s.iptalSaati).length,gelen=satirlar.filter(s=>s.geldi).length;

  const icerik=(
    <div ref={kapRef} style={tamEkran?{position:"fixed",inset:0,zIndex:2000,background:"#f5f4f1",overflowY:"auto",padding:"1.25rem 2rem"}:{}}>
      {/* Üst bar */}
      <div style={{display:"flex",alignItems:"center",gap:10,flexWrap:"wrap",marginBottom:12}}>
        <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
          {gorunenOdalar.map(([k,l])=>{
            const L=(listeler[k]||[]).filter(s=>!s.iptalSaati),g=L.filter(s=>s.geldi).length;
            const aktif=aktifOda===k;
            return <button key={k} onClick={()=>setAktifOda(k)} style={{padding:`${10*fs}px ${18*fs}px`,borderRadius:10,border:aktif?"2px solid #6366f1":"1px solid #ddd",background:aktif?"#eef0ff":"#fff",color:aktif?"#4338ca":"#444",fontWeight:600,fontSize:14*fs,cursor:"pointer",fontFamily:"inherit"}}>
              {l} <span style={{fontWeight:400,opacity:0.7}}>{g}/{L.length}</span>
            </button>;
          })}
        </div>
        <div style={{flex:1}}/>
        {!tamEkran&&<div style={{display:"flex",alignItems:"center",gap:4}}>
          <button onClick={()=>setTarih(addDays(tarih,-1))} style={navBtnStyle}>‹</button>
          <input type="date" value={tarih} onChange={e=>e.target.value&&setTarih(e.target.value)} style={{...inputStyle,width:150,padding:"6px 8px"}}/>
          <button onClick={()=>setTarih(addDays(tarih,1))} style={navBtnStyle}>›</button>
          {!bugunMu&&<button onClick={()=>setTarih(today())} style={navBtnStyle}>Bugün</button>}
        </div>}
        <span title={canli?"Anlık bağlantı açık":"Anlık bağlantı yok — 20 sn'de bir yenileniyor"} style={{fontSize:12*fs,color:canli?"#16a34a":"#b45309",display:"flex",alignItems:"center",gap:5}}>
          <span style={{width:8,height:8,borderRadius:8,background:canli?"#22c55e":"#f59e0b"}}/>{canli?"Canlı":"Yedek mod"}
        </span>
        <span style={{fontSize:20*fs,fontWeight:600,fontVariantNumeric:"tabular-nums",color:"#1a1a2e"}}>{saatSimdi}</span>
        {tamEkran
          ?<button onClick={tamEkranKapat} style={{...btnSecondary,fontSize:14*fs,background:"#fff"}}>✕ Kapat</button>
          :<>
            <button onClick={()=>listeYazdir("lazer")} title="Alex + Soprano listesini yazdır" style={{...btnSecondary,background:"#fff"}}>🖨 Alex/Soprano</button>
            <button onClick={()=>listeYazdir("dr")} title="Dr İşlem listesini yazdır" style={{...btnSecondary,background:"#fff"}}>🖨 Dr İşlem</button>
            <button onClick={tamEkranAc} style={{...btnSecondary,background:"#fff"}}>⛶ Tam Ekran</button>
          </>}
      </div>

      {tamEkran&&<div style={{fontSize:14*fs,color:"#666",marginBottom:8}}>{formatDate(tarih)} · Gelen {gelen}/{toplam}</div>}

      {!tamEkran&&(
        <div style={{display:"flex",gap:8,alignItems:"center",flexWrap:"wrap",marginBottom:10}}>
          <div style={{display:"inline-flex",border:"1px solid #ddd",borderRadius:8,overflow:"hidden",background:"#fff"}}>
            {[["randevu","Randevu saatine göre"],["gelis","Geliş sırasına göre"]].map(([k,l])=>(
              <button key={k} onClick={()=>siralaDegistir(k)} style={{padding:"6px 12px",border:"none",background:siralama===k?"#eef0ff":"transparent",color:siralama===k?"#4338ca":"#555",fontWeight:siralama===k?600:400,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>{l}</button>
            ))}
          </div>
          {(gelisIsaretleyebilir||drDuzenleyebilir)&&!randevusuz.acik&&aktifOda!=="tumu"&&<button onClick={()=>setRandevusuz(r=>({...r,acik:true}))} style={{...btnSecondary,padding:"6px 12px",fontSize:13,background:"#fff"}}>＋ Randevusuz hasta geldi</button>}
          {randevusuz.acik&&(
            <form onSubmit={randevusuzEkle} style={{display:"flex",gap:6,alignItems:"center",flexWrap:"wrap",padding:"6px 8px",background:"#faf5ff",border:"1px solid #ddd6fe",borderRadius:8}}>
              <span style={{fontSize:12,color:"#6d28d9",fontWeight:600}}>{BH_ODALAR.find(o=>o[0]===aktifOda)?.[1]} · şimdi geldi:</span>
              <input autoFocus placeholder="Hasta adı soyadı" value={randevusuz.hasta} onChange={e=>setRandevusuz(r=>({...r,hasta:e.target.value}))} style={{...inputStyle,width:200,padding:"6px 10px"}}/>
              <input placeholder="İşlem (Lazer, Forma…)" value={randevusuz.islem} onChange={e=>setRandevusuz(r=>({...r,islem:e.target.value}))} style={{...inputStyle,width:170,padding:"6px 10px"}}/>
              <button type="submit" disabled={ekleniyor} style={{...btnPrimary,padding:"6px 14px",background:"#7c3aed"}}>Ekle</button>
              <button type="button" onClick={()=>setRandevusuz({acik:false,hasta:"",islem:""})} style={{...btnSecondary,padding:"6px 10px"}}>Vazgeç</button>
            </form>
          )}
        </div>
      )}

      {/* Dr İşlem — sekreter doktor takviminden elle girer */}
      {aktifOda==="dr"&&drDuzenleyebilir&&!tamEkran&&(
        <form onSubmit={manuelEkle} style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center",marginBottom:10,padding:"10px 12px",background:"#fff",border:"1px dashed #cbd5e1",borderRadius:10}}>
          <span style={{fontSize:13,fontWeight:600,color:"#555",marginRight:4}}>＋ Dr randevusu:</span>
          <input ref={saatInputRef} type="time" step={300} required value={manuel.saat} onChange={e=>setManuel(m=>({...m,saat:e.target.value}))} style={{...inputStyle,width:115}}/>
          <input placeholder="Hasta adı soyadı" required value={manuel.hasta} onChange={e=>setManuel(m=>({...m,hasta:e.target.value}))} style={{...inputStyle,flex:2,minWidth:170}}/>
          <input placeholder="İşlem (Botoks, Dolgu…)" value={manuel.islem} onChange={e=>setManuel(m=>({...m,islem:e.target.value}))} style={{...inputStyle,flex:1,minWidth:130}}/>
          <select value={manuel.sure} onChange={e=>setManuel(m=>({...m,sure:e.target.value}))} style={{...inputStyle,width:95}}>
            <option value="">Süre</option>{BH_SURELER.map(d=><option key={d} value={d}>{d} dk</option>)}
          </select>
          <input placeholder="Not" value={manuel.notlar} onChange={e=>setManuel(m=>({...m,notlar:e.target.value}))} style={{...inputStyle,flex:1,minWidth:100}}/>
          <button type="submit" disabled={ekleniyor} style={btnPrimary}>{ekleniyor?"…":"Ekle"}</button>
          <span style={{width:1,alignSelf:"stretch",background:"#e5e7eb",margin:"0 4px"}}/>
          <label title="DoktorTakvimi → Randevu listesi → Yazdır → 'PDF olarak kaydet' (saatli) ya da Randevulara genel bakış → Excel" style={{...btnSecondary,background:"#f0fdf4",borderColor:"#86efac",color:"#166534",fontWeight:600,display:"inline-block"}}>
            📥 DoktorTakvimi (PDF / Excel)<input type="file" accept=".pdf,application/pdf,.xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={excelSec} style={{display:"none"}}/>
          </label>
          {listeler.dr.length>0&&<button type="button" onClick={drListeSil} title="Bu günün Dr İşlem listesinin tamamını sil" style={{...btnSecondary,background:"#fff",color:"#b91c1c",border:"1px solid #fca5a5",fontWeight:600}}>🗑 Tüm listeyi sil</button>}
        </form>
      )}
      ...
      <div style={{background:"#fff",borderRadius:14,border:"1px solid #e8e6e0",overflow:"hidden"}}>
        <div style={{overflowX:"auto"}}>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:14*fs}}>
            <thead>
              <tr style={{background:"#fafaf8",color:"#888",fontSize:11*fs,textTransform:"uppercase",letterSpacing:0.5,textAlign:"left"}}>
                <th style={{padding:"10px 14px",width:70*fs}}>Saat</th>
                <th style={{padding:"10px 14px"}}>Hasta</th>
                <th style={{padding:"10px 14px"}}>{aktifOda==="dr"?"İşlem":aktifOda==="tumu"?"Bölge / İşlem":"Bölge"}</th>
                <th style={{padding:"10px 14px",width:130*fs}}>Durum</th>
                <th style={{padding:"10px 14px",width:80*fs}}>Geliş</th>
                {(aktifOda==="dr"||aktifOda==="tumu")&&<th style={{padding:"10px 14px",width:200*fs}}>K. saati</th>}
                <th style={{padding:"10px 14px",width:190*fs}}></th>
              </tr>
            </thead>
            <tbody>
              {yukleniyor&&satirlar.length===0&&<tr><td colSpan={8} style={{padding:24,textAlign:"center",color:"#aaa"}}>Yükleniyor…</td></tr>}
              {!yukleniyor&&satirlar.length===0&&<tr><td colSpan={8} style={{padding:28,textAlign:"center",color:"#aaa"}}>
                {aktifOda==="dr"?"Bu gün için Dr İşlem listesi yok. Doktor takvimindeki randevuları yukarıdan ekleyin.":aktifOda==="tumu"?"Bu gün için hasta yok.":"Bu gün bu odada randevu yok."}
              </td></tr>}
              {satirlar.map(s=>{
                const d=satirDurumu(s);
                const kd=s.oda==="dr"?kremDurum(s):null;
                const iptal=!!s.iptalSaati,soluk=iptal||s.randevuDurum==="Gelmedi";
                const hucre={padding:`${10*fs}px 14px`};
                return(
                  <tr key={s.key} style={{background:d.satirBg,borderTop:"1px solid #f0efeb",transition:"background 0.3s",opacity:soluk?0.65:1,color:s.alinmaSaati?"#374151":iptal?"#9f1239":"inherit"}}>
                    <td style={{...hucre,fontWeight:600,fontVariantNumeric:"tabular-nums"}}>{s.randevusuz?"—":s.saat}</td>
                    <td style={{...hucre,fontWeight:600}}>
                      {aktifOda==="tumu"&&<BhOdaRozet oda={s.oda}/>}<span style={{textDecoration:iptal?"line-through":"none"}}>{s.hasta}</span>
                      {s.randevusuz&&<span style={{fontSize:10,fontWeight:700,color:"#7c3aed",background:"#ede9fe",padding:"1px 5px",borderRadius:4,marginLeft:6}}>Randevusuz</span>}
                      {!tamEkran&&(s.notlar||s.tel)&&<div style={{fontSize:11,color:"#999",fontWeight:400,marginTop:2}}>{[s.tel&&"📞 "+s.tel,s.notlar].filter(Boolean).join(" · ")}</div>}
                    </td>
                    <td style={{...hucre,color:iptal?"inherit":"#555",fontSize:13*fs}}>
                      {s.randevuDurum==="Rütuş"&&<span style={{fontSize:11,fontWeight:700,color:"#a16207",background:"#fef3c7",padding:"1px 6px",borderRadius:4,marginRight:6}}>Rütuş</span>}
                      {s.islem||"—"}
                    </td>
                    ...
                    <td style={{padding:`${8*fs}px 14px`,textAlign:"right",whiteSpace:"nowrap"}}>
                      {gelisIsaretleyebilir&&!s.geldi&&!soluk&&<>
                      ...
                      {!tamEkran&&<>
                        ...
                        {(s.oda==="dr"||s.randevusuz)&&drDuzenleyebilir&&<button onClick={()=>drSil(s)} title="Listeden sil" style={{marginLeft:6,background:"none",border:"none",color:"#dc2626",cursor:"pointer",fontSize:15}}>✕</button>}
                      </>}
                    </td>
                  </tr>
                );
      ...
      {/* DoktorTakvimi Excel önizleme — saat ve süre sekreter tarafından doldurulur */}
      {excelOnizleme&&(()=>{
        const S=excelOnizleme.satirlar,sec=S.filter(s=>s.sec),saatli=sec.filter(s=>bhSaatDuzelt(s.saat)).length;
        const dosyaTarih=[...new Set(S.map(s=>s.tarih).filter(Boolean))];
        const hucre={padding:"5px 6px",borderTop:"1px solid #f0efeb",verticalAlign:"middle"};
        const kucuk={...inputStyle,padding:"6px 8px",fontSize:13};
        return(
          <div onClick={()=>setExcelOnizleme(null)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.45)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:1000,padding:16}}>
            <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:16,padding:"1.25rem",width:"100%",maxWidth:1050,maxHeight:"92vh",display:"flex",flexDirection:"column",boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
              <div style={modalTitleStyle}>📥 DoktorTakvimi — {dosyaTarih.map(t=>formatDate(t)).join(", ")||excelOnizleme.dosya}</div>
              <div style={{fontSize:13,color:"#555",marginBottom:8,lineHeight:1.5}}>
                {excelOnizleme.saatli
                  ?<>Saatler dosyadan okundu. Kontrol et, istersen <b>süre</b> ekle ve kaydet.</>
                  :<>Dosyada randevu saati yok. Doktor takvimine bakarak her hastanın <b>saatini</b>, istersen <b>süresini</b> gir. Sıra dosyadaki gibidir.</>}
                {dosyaTarih.length===1&&dosyaTarih[0]!==tarih&&<div style={{color:"#b45309",marginTop:4}}>⚠️ Dosya {formatDate(dosyaTarih[0])} tarihli. Kayıtlar o güne eklenecek.</div>}
              </div>
              <div style={{overflowY:"auto",border:"1px solid #eee",borderRadius:10,flex:1}}>
                <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
                  <thead><tr style={{background:"#fafaf8",color:"#888",fontSize:11,textTransform:"uppercase",textAlign:"left",position:"sticky",top:0}}>
                    <th style={{padding:"8px 6px",width:30}}><input type="checkbox" checked={S.every(s=>s.sec||s.iptal||s.mevcut)&&sec.length>0} onChange={e=>setExcelOnizleme(o=>({...o,satirlar:o.satirlar.map(s=>s.iptal||s.mevcut?s:{...s,sec:e.target.checked})}))}/></th>
                    <th style={{padding:"8px 6px",width:115}}>Saat *</th><th style={{padding:"8px 6px"}}>Hasta</th><th style={{padding:"8px 6px"}}>İşlem</th><th style={{padding:"8px 6px"}}>Not</th><th style={{padding:"8px 6px",width:95}}>Süre</th><th style={{padding:"8px 6px",width:90}}></th>
                  </tr></thead>
                  <tbody>{S.map(s=>(
                    <tr key={s.id} style={{opacity:s.sec?1:0.45,background:s.sec&&!bhSaatDuzelt(s.saat)?"#fffbeb":"#fff"}}>
                      <td style={hucre}><input type="checkbox" checked={s.sec} onChange={e=>excelSatirGuncelle(s.id,"sec",e.target.checked)}/></td>
                      <td style={hucre}><input type="time" step={300} value={s.saat} disabled={!s.sec} onChange={e=>excelSatirGuncelle(s.id,"saat",e.target.value)} style={{...kucuk,borderColor:s.sec&&!bhSaatDuzelt(s.saat)?"#f59e0b":"#ddd"}}/></td>
                      <td style={hucre}><input value={s.hasta} disabled={!s.sec} onChange={e=>excelSatirGuncelle(s.id,"hasta",e.target.value)} style={{...kucuk,fontWeight:600}}/>{s.tel&&<div style={{fontSize:11,color:"#999",marginTop:2}}>📞 {s.tel}</div>}</td>
                      <td style={hucre}><input value={s.islem} disabled={!s.sec} placeholder="İşlem" onChange={e=>excelSatirGuncelle(s.id,"islem",e.target.value)} style={kucuk}/></td>
                      <td style={hucre}><input value={s.notlar||""} disabled={!s.sec} placeholder="Not" onChange={e=>excelSatirGuncelle(s.id,"notlar",e.target.value)} style={{...kucuk,fontSize:12}}/></td>
                      <td style={hucre}><select value={s.sure} disabled={!s.sec} onChange={e=>excelSatirGuncelle(s.id,"sure",e.target.value)} style={kucuk}><option value="">—</option>{BH_SURELER.map(d=><option key={d} value={d}>{d} dk</option>)}</select></td>
                      <td style={{...hucre,fontSize:11,whiteSpace:"nowrap"}}>
                        {s.iptal&&<span style={{color:"#991b1b",background:"#fee2e2",padding:"2px 6px",borderRadius:4}}>{s.durum}</span>}
                        {s.mevcut&&<span style={{color:"#1e40af",background:"#dbeafe",padding:"2px 6px",borderRadius:4}}>Zaten listede</span>}
                        {!s.tarih&&<span style={{color:"#991b1b"}}>Tarih okunamadı</span>}
                      </td>
                    </tr>))}</tbody>
                </table>
              </div>
              <div style={{display:"flex",gap:8,justifyContent:"space-between",alignItems:"center",marginTop:12,flexWrap:"wrap"}}>
                <span style={{fontSize:13,color:saatli===sec.length?"#166534":"#b45309",fontWeight:600}}>Saat girilen: {saatli}/{sec.length}</span>
                <div style={{display:"flex",gap:8}}>
                  <button onClick={()=>setExcelOnizleme(null)} style={btnSecondary}>Vazgeç</button>
                  <button onClick={excelKaydet} disabled={ekleniyor||sec.length===0} style={{...btnPrimary,opacity:saatli===sec.length&&sec.length>0?1:0.6}}>{ekleniyor?"Kaydediliyor…":`${sec.length} hastayı ekle`}</button>
                </div>
              </div>
            </div>
          </div>
